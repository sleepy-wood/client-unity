            ...
            foreach (HalfEdgeVertex3 vertex in vertices)
            {
                ...
                if (edgesGoingToVertex.Count == 3)
                {
                    ...
                    if (distance < FLAT_TETRAHEDRON_DISTANCE)
                    {
                        ...
                        Vector3 p1 = MyVector3.ToVector3(normalizer.UnNormalize(v1.position));
                        Vector3 p2 = MyVector3.ToVector3(normalizer.UnNormalize(v2.position));
                        Vector3 p3 = MyVector3.ToVector3(normalizer.UnNormalize(v3.position));
