...
namespace Broccoli.NodeEditorFramework
{
	public class NodeEditorUserCache
	{
		...
		private string lastSessionPath { get { return _cachePath + "/LastSession.asset"; } }
		...
		private void SaveNewNode (Node node) 
		{
			...
			ScriptableObject[] sos = node.GetScriptableObjects ();
			for (int i = 0; i < sos.Length; i++) {
				NodeEditorSaveManager.AddSubAsset (sos [i], node);
			}

			for (int i = 0; i < node.nodeKnobs.Count; i++) {
				NodeEditorSaveManager.AddSubAsset (node.nodeKnobs[i], node);
				sos = node.nodeKnobs [i].GetScriptableObjects ();
				for (int j = 0; j < sos.Length; j++) {
					NodeEditorSaveManager.AddSubAsset (sos [j], node.nodeKnobs [i]);
				}
			}
		...
		private void SaveNewNodeKnob (NodeKnob knob) 
		{
			...
			ScriptableObject[] sos = knob.GetScriptableObjects ();
			for (int i = 0; i < sos.Length; i++) {
				NodeEditorSaveManager.AddSubAsset (sos [i], knob);
			}
