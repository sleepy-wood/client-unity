...
namespace Broccoli.NodeEditorFramework
{
	...
	public partial class NodeInput : NodeKnob
	{
		...
		public TypeData typeData { get { CheckType (); return _typeData; } }
...
//				connections[conCnt] = replaceSerializableObject.Invoke (connections[conCnt]) as NodeOutput;
		...
		/// </summary>
		public bool TryApplyConnection (NodeOutput output, bool issueEvents = true)
		{
			if (CanApplyConnection (output)) 
			{ // It can connect (type is equals, it does not cause recursion, ...)
				ApplyConnection (output, issueEvents);
				return true;
			}
			return false;
		}
...
//				Debug.LogError ("Cannot assign " + typeData.Type.ToString () + " to " + output.typeData.Type.ToString ());
		...
		/// </summary>
		public void ApplyConnection (NodeOutput output, bool issueEvents = true)
		{
			if (output == null) 
				return;
			...
			{
				if (issueEvents) {
					NodeEditorCallbacks.IssueOnRemoveConnection (this, body.canvas.GetType ());
				}
			...
			if (issueEvents) {
				NodeEditorCallbacks.IssueOnAddConnection (this, body.canvas.GetType ());
			}
		...
		/// </summary>
		public void RemoveConnection (bool issueOnRemove = true)
		{
			if (connection == null)
				return;
			if (issueOnRemove) {
				NodeEditorCallbacks.IssueOnRemoveConnection (this, this.body.canvas.GetType ());
			}
			connection.connections.Remove (this);
			connection = null;
