...
namespace Broccoli.NodeEditorFramework
{
	...
	public static partial class NodeEditor 
	{
		...
		{
		#if UNITY_EDITOR
			string[] assets = UnityEditor.AssetDatabase.FindAssets ("BroccoliNodeEditorCallbackReceiver"); // Something relatively unique
			if (assets.Length != 1) 
			{
		...
		private static void DrawSubCanvas (NodeCanvas nodeCanvas, NodeEditorState editorState)  
		{
			...
			if (nodeCanvas.zoomEnabled) {
				...
				curEditorState.zoomPanAdjust = curEditorState.zoomPos;
			}
			...
			{ // Draw the currently drawn connection from NodeOutput
				...
				RepaintClients ();
			} else if (curEditorState.connectInput != null) {
				NodeInput input = curEditorState.connectInput;
				Vector2 startPos = input.GetGUIKnob ().center;
				Vector2 startDir = input.GetDirection ();
				Vector2 endPos = Event.current.mousePosition;
				Vector2 endDir = -startDir; // NodeEditorGUI.GetSecondConnectionVector (startPos, endPos, startDir); <- causes unpleasant jumping when switching polarity

				NodeEditorGUI.OptimiseBezierDirections (startPos, ref startDir, endPos, ref endDir);
				NodeEditorGUI.DrawConnection (startPos, startDir, endPos, endDir, input.typeData.Color, input.typeData.LineWidth);
				RepaintClients ();
			}
