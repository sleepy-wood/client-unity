...
namespace Broccoli.NodeEditorFramework
{
	public abstract partial class Node : ScriptableObject
	{
		...
		protected Color lastBGColor = Color.white;
		protected GUIStyle nodeBGStyle;
		...
		protected internal void InitBase () 
		{
			if (!NodeEditor.curNodeCanvas.nodes.Contains (this)) {
				...
				this.canvas = NodeEditor.curNodeCanvas;
			}
		...
		public static Node Create (string nodeID, Vector2 position, NodeOutput connectingOutput) 
		{
			...
			{ // Handle auto-connection and link the output to the first compatible input
				...
				{
					if (node.Inputs[i].TryApplyConnection (connectingOutput))
						break;
				}
			}
		...
		private void AssureNodeBGStyle ()
		{
			if (nodeBGStyle == null || nodeBGStyle.normal.background == null || lastBGColor != backgroundColor)
			{
				...
				nodeBGStyle.normal.background = ResourceManager.GetTintedTexture ("Broccoli/GUI/NE_Box.png", backgroundColor);
			}
		}
		...
		protected static void ReassignOutputType (ref NodeOutput output, Type newOutputType) 
		{
			...
			var validConnectionsEnumerator = validConnections.GetEnumerator ();
			while (validConnectionsEnumerator.MoveNext ()) {
				validConnectionsEnumerator.Current.ApplyConnection (output);
			}
		}
