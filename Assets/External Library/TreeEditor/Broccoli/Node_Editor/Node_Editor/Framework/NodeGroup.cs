...
namespace Broccoli.NodeEditorFramework
{
	...
	public class NodeGroup
	{
		...
		private void AddPins ()
		{
			...
			List<Node> nodes = NodeEditor.curNodeCanvas.nodes;
			for (int i = 0; i < nodes.Count; i++)
			{ // Get all pinned nodes -> all nodes atleast half in the group
				if (rect.Contains (nodes[i].rect.center) && !pinnedNodes.Contains (nodes[i]))
					pinnedNodes.Add (nodes[i]);
			}
		}
			...
			{ // Handle dragging and resizing of active group
				...
				{ // Dragging -> Apply drag to body and pinned nodes
					...
					for (int i = 0; i < pinnedNodes.Count; i++) {
						pinnedNodes[i].rect.position += dragChange;
					}
					for (int i = 0; i < pinnedGroups.Count; i++) {
						pinnedGroups[i].rect.position += dragChange;
					}
				}
				inputInfo.inputEvent.Use ();
