	...
	public static class NodeEditorInputSystem
	{
		...
		public static void SetupInput () 
		{
			...
			while (scriptAssembliesEnumerator.MoveNext ())
			{
				...
				{
					MethodInfo[] methods = types [i].GetMethods (BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
					for (int j = 0; j < methods.Length; j++)
					{
						...
						{
							Type attrType = attributes[k].GetType ();
							if (attrType == typeof(EventHandlerAttribute))
							{ // Method is an eventHandler
								if (EventHandlerAttribute.AssureValidity (methods[j], attributes[k] as EventHandlerAttribute)) 
								{ // Method signature is correct, so we register this handler
									if (actionDelegate == null) actionDelegate = Delegate.CreateDelegate (typeof(Action<NodeEditorInputInfo>), methods[j]);
									eventHandlers.Add (new KeyValuePair<EventHandlerAttribute, Delegate> (attributes[k] as EventHandlerAttribute, actionDelegate));
								}
							}
							else if (attrType == typeof(HotkeyAttribute))
							{ // Method is an hotkeyHandler
								if (HotkeyAttribute.AssureValidity (methods[j], attributes[k] as HotkeyAttribute)) 
								{ // Method signature is correct, so we register this handler
									if (actionDelegate == null) actionDelegate = Delegate.CreateDelegate (typeof(Action<NodeEditorInputInfo>), methods[j]);
									hotkeyHandlers.Add (new KeyValuePair<HotkeyAttribute, Delegate> (attributes[k] as HotkeyAttribute, actionDelegate));
								}
							}
							else if (attrType == typeof(ContextEntryAttribute))
							{ // Method is an contextEntry
								if (ContextEntryAttribute.AssureValidity (methods[j], attributes[k] as ContextEntryAttribute)) 
								{ // Method signature is correct, so we register this handler
									if (actionDelegate == null) actionDelegate = Delegate.CreateDelegate (typeof(Action<NodeEditorInputInfo>), methods[j]);
									...
									contextEntries.Add (new KeyValuePair<ContextEntryAttribute, PopupMenu.MenuFunctionData> (attributes[k] as ContextEntryAttribute, menuFunction));
								}
							}
							else if (attrType == typeof(ContextFillerAttribute))
							{ // Method is an contextFiller
								if (ContextFillerAttribute.AssureValidity (methods[j], attributes[k] as ContextFillerAttribute)) 
								{ // Method signature is correct, so we register this handler
									Delegate methodDel = Delegate.CreateDelegate (typeof(Action<NodeEditorInputInfo, GenericMenu>), methods[j]);
									contextFillers.Add (new KeyValuePair<ContextFillerAttribute, Delegate> (attributes[k] as ContextFillerAttribute, methodDel));
								}
							}
						}
		...
		private static void CallEventHandlers (NodeEditorInputInfo inputInfo, bool late) 
		{
			...
			var eventHandlersEnumerator = eventHandlers.GetEnumerator ();
			while (eventHandlersEnumerator.MoveNext ())
			{
				var eventHandlerPair = eventHandlersEnumerator.Current;
				if ((eventHandlerPair.Key.handledEvent == null || eventHandlerPair.Key.handledEvent == inputInfo.inputEvent.type) &&
					(late? eventHandlerPair.Key.priority >= 100 : eventHandlerPair.Key.priority < 100))
				{ // Event is happening and specified priority is ok with the late-state
					eventHandlerPair.Value.DynamicInvoke (parameter);
		...
		{
			object[] parameter = new object[] { inputInfo };
			var hotkeyHandlersEnumerator = hotkeyHandlers.GetEnumerator ();
			while (hotkeyHandlersEnumerator.MoveNext ())
			{
				var hotKeyPair = hotkeyHandlersEnumerator.Current;
				if (hotKeyPair.Key.handledHotKey == keyCode && 
					(hotKeyPair.Key.modifiers == null || hotKeyPair.Key.modifiers == mods) && 
					(hotKeyPair.Key.limitingEventType == null || hotKeyPair.Key.limitingEventType == inputInfo.inputEvent.type))
				{
					hotKeyPair.Value.DynamicInvoke (parameter);
					if (inputInfo.inputEvent.type == EventType.Used)
						return;
				}
			}
		...
		{
			var contextEntriesEnumerator = contextEntries.GetEnumerator ();
			while (contextEntriesEnumerator.MoveNext ())
			{ // Add all registered menu entries for the specified type to the contextMenu
				var contextEntryPair = contextEntriesEnumerator.Current;
				if (contextEntryPair.Key.contextType == contextType)
					contextMenu.AddItem (new GUIContent (contextEntryPair.Key.contextPath), false, contextEntryPair.Value, inputInfo);
			}

			object[] fillerParams = new object[] { inputInfo, contextMenu };
			var contextFillersEnumerator = contextFillers.GetEnumerator ();
			while (contextFillersEnumerator.MoveNext ())
			{ // Let all registered menu fillers for the specified type add their entries to the contextMenu
				var contextFillerPair = contextFillersEnumerator.Current;
				if (contextFillerPair.Key.contextType == contextType)
					contextFillerPair.Value.DynamicInvoke (fillerParams);
			}
		}
		...
		private static void HandleSelecting (NodeEditorInputInfo inputInfo) 
		{
			...
			{ // Select focussed Node
				...
				if (state.selectedNode != null) {
					NodeEditorCallbacks.IssueOnSelectNode (state.selectedNode, state.canvas.GetType());
				} else {
					NodeEditorCallbacks.IssueOnDeselectNode (state.canvas.GetType());
				}
				#endif
			}
		}
		...
		private static void HandleContextClicks (NodeEditorInputInfo inputInfo) 
		{
			...
			{ // Handle context clicks on Node and canvas
				...
				NodeEditorState state = inputInfo.editorState;
				if (inputInfo.editorState.focusedNode != null && state.canvas.showNodeMenuEnabled) { // Node Context Click
					FillContextMenu (inputInfo, contextMenu, ContextType.Node);
					contextMenu.Show (inputInfo.inputPos);
				} else if (state.canvas.showCanvasMenuEnabled) { // Editor Context Click
